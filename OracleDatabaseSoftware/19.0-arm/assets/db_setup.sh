...
echo "export ORACLE_HOME=\$ORACLE_BASE/product/19.0.0/dbhome" >> /.oracle_env
...
wget -q --no-check-certificate "https://github.com/tianon/gosu/releases/download/1.16/gosu-arm64"  -O /usr/local/bin/gosu
chmod +x /usr/local/bin/gosu

# download and extract Oracle database software
echo "downloading Oracle database software..."
wget -q --no-check-certificate ${ORACLE_ASSETS}/arm/LINUX.ARM64_1919000_db_home.zip -O /tmp/oracle/db1.zip
...
# download and extract Oracle sample schemas (latest version of "old" example schemas)
echo "downloading Oracle sample schemas..."
wget -q --no-check-certificate https://github.com/oracle/db-sample-schemas/archive/3b12adfc4859cfdafc3363bc4992a461ba8ddb8f.zip -O /tmp/db-sample-schemas-main.zip
echo "extracting Oracle sample schemas..."
unzip /tmp/db-sample-schemas-main.zip -d ${ORACLE_HOME}/demo/ > /dev/null
mv ${ORACLE_HOME}/demo/db-sample-schemas-3b12adfc4859cfdafc3363bc4992a461ba8ddb8f ${ORACLE_HOME}/demo/schema
...
# currently there is no RU available for ARM. It's just 19.19.
# once they become available, extend this section accordingly
...
wget -q --no-check-certificate ${ORACLE_ASSETS}/apex_23.1_en.zip -O /tmp/apex.zip
...
wget -q --no-check-certificate ${ORACLE_ASSETS}/ords-23.2.3.242.1937.zip -O /tmp/ords.zip
