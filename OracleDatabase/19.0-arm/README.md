# Oracle Database 19c Enterprise Edition (ARM)
...
* Oracle Linux 8.8
* Oracle Database 19c Enterprise Edition Release 19.0.0.0.0 - Production Version 19.19.0.0.0
	* Sample schemas SCOTT, HR, OE, PM, IX, SH, BI (old examples as of 2023-04-03, before 23c update)
	* APEX 23.1.5 including APEX\_LISTENER and APEX\_REST\_PUBLIC\_USER
...
1. Create the container
		...
		docker run -d -p 8080-8081:8080-8081 -p 1521:1521 -h odb --name odb phsalvisberg/odb:19.0-arm

2. wait around **10 minutes** until the Oracle database instance is created and APEX is updated to the latest version. Check logs with ```docker logs -f -t odb```. The container is ready to use when the last line in the log is ```Database ready to use. Enjoy! ;-)```. The container stops if an error occurs. Check the logs to determine how to proceed.
...
```
docker run -e PASS=manager -e APEX=false -d -p 8080-8081:8080-8081 -p 1521:1521 -h odb --name odb phsalvisberg/odb:19.0-arm
```
...
```
docker run -e MULTITENANT=true -d -p 8080-8081:8080-8081 -p 1521-1523:1521-1523 -h ocdb --name ocdb phsalvisberg/odb:19.0-arm
```
...
```
docker run -v odb:/u02 -d -p 8080-8081:8080-8081 -p 1521:1521 -h odb --name odb phsalvisberg/odb:19.0-arm
```

Here's an example mapping the local directory ```$HOME/docker/odb/u02``` to ```/u02```.

```
docker run -v $HOME/docker/odb/u02:/u02 -d -p 8080-8081:8080-8081 -p 1521:1521 -h odb --name odb phsalvisberg/odb:19.0-arm
```
...
- when using ORDS: [http://localhost:8081/ords/apex](http://localhost:8081/ords/apex)
