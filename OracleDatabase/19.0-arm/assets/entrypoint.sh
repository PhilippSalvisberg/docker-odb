...
link_dir_to_volume(){
	...
	rm -rf ${LINK}
	mkdir -p ${TARGET}
	chown -fR oracle:dba ${TARGET} || true
	echo "Link ${LINK} to ${TARGET}."
	ln -s ${TARGET} ${LINK}
	chown -fR oracle:dba ${LINK} || true
}

provide_data_as_single_volume(){
	echo "Providing persistent data under /u02 to be used as Docker volume"
	link_dir_to_volume "/u01/app/oracle/product/19.0.0/dbhome/dbs" "/u02/app/oracle/product/19.0.0/dbhome/dbs" 
